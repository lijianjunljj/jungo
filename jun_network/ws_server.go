...
func (handler *WSHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		jun_log.Debug("upgrade error: %v", err)
		return
	}
	...
	if len(handler.conns) >= handler.maxConnNum {
		handler.mutexConns.Unlock()
		conn.Close()
		jun_log.Debug("too many connections")
		return
	}
...
func (server *WSServer) Start() {
	...
	if server.MaxConnNum <= 0 {
		...
		jun_log.Release("invalid MaxConnNum, reset to %v", server.MaxConnNum)
	}
	if server.PendingWriteNum <= 0 {
		server.PendingWriteNum = 100
		jun_log.Release("invalid PendingWriteNum, reset to %v", server.PendingWriteNum)
	}
	if server.MaxMsgLen <= 0 {
		server.MaxMsgLen = 4096
		jun_log.Release("invalid MaxMsgLen, reset to %v", server.MaxMsgLen)
	}
