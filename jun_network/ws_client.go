...
func (client *WSClient) init() {
	...
	if client.ConnNum <= 0 {
		client.ConnNum = 1
		jun_log.Release("invalid ConnNum, reset to %v", client.ConnNum)
	}
	if client.ConnectInterval <= 0 {
		client.ConnectInterval = 3 * time.Second
		jun_log.Release("invalid ConnectInterval, reset to %v", client.ConnectInterval)
	}
	...
	if client.HandshakeTimeout <= 0 {
		...
		jun_log.Release("invalid HandshakeTimeout, reset to %v", client.HandshakeTimeout)
	}
...
func (client *WSClient) dial() *websocket.Conn {
	for {
		...
		jun_log.Release("connect to %v error: %v", client.Addr, err)
